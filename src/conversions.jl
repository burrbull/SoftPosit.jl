...
function posit(::Type{PositN}, x::FloatN) where {PositN<:AbstractPosit, FloatN<:Base.IEEEFloat}
    ...
    regime = reinterpret(IntN,Base.sign_mask(FloatN) >> signbit_e)
    ...
    exponent = e & Base.exponent_mask(PositN)
    exponent <<= bitsize(FloatN)-2-Base.exponent_bits(PositN)
    ...
    p = bitround(Base.uinttype(PositN),reinterpret(UIntN,regime_exponent_mantissa))
